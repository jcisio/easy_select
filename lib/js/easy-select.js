...
(function($) {
  ...
  var renderChildren = function(value) {
    ...
    while (start < choices.length) {
      ...
      if (thisLevel === currentLevel) {
        children[element.id] = '<span class="easy-select-choice"><input type="checkbox" value="' + element.value + '" id="' + element.id + '"/><label for="' + element.id + '">' + element.label.substr(currentLevel) + '</label></span>';
      }
  ...
  $('input').live('click', function() {
    ...
    // If not a root element, do not move.
    ...
    if (e.parents('.easy-select-choices').length) {
      ...
      e.parent().find('input[checked]').click();
      e.parent().appendTo($('#' + e.attr('id') + '-choices'));
    }
  });
